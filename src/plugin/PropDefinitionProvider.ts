 ...
 * @LastEditors: E'vils
 * @LastEditTime: 2021-07-20 16:51:42
 * @Description: 
 * @FilePath: /src/plugin/PropDefinitionProvider.ts
 */
...
export class PropDefinitionProvider implements DefinitionProvider {
  ...
  public async provideDefinition(
    ...
    if (tag) {
      ...
      const rawAttrValue = ((attrs["__" + attrName] || "") as string).replace(
        /^['"]|['"]$/g,
        ""
      ); // 去除引号
      ...
      if (attrName === "class") {
        ...
        attrName.endsWith(".sync") ||
        (rawAttrValue.startsWith("{{") && rawAttrValue.endsWith("}}"))
      ) {
        return this.searchScript("prop", posWord, document);
      } else if (
        /^(bind|catch)/.test(attrName) ||
      ...
      styfile.styles.forEach((sty) => {
        if (sty.name === className) {
          ...
          let end = new Position(
            start.line,
            1 + start.character + className.length
          );
          locs.push(
            new Location(Uri.file(styfile.file), new Range(start, end))
          );
        }
      });
    });
