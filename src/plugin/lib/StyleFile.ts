 ...
 * @LastEditors: E'vils
 * @LastEditTime: 2021-07-20 17:39:26
 * @Description:
 * @FilePath: /src/plugin/lib/StyleFile.ts
 */
...
export function parseStyleFile(file: string) {
  try {
    ...
    if (editor) {
      ...
      return {
        file,
        styles: quickParseStyle(
          isScss(file) ? loadScss({ data: content, file }) : content
        ),
      };
    } else {
      ...
      cache = {
        ...
        value: {
          file,
          styles: quickParseStyle(
            isScss(file) ? loadScss({ file }) : fs.readFileSync(file).toString()
          ),
        },
      };
