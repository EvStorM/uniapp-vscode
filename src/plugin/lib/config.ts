...
function getConfig() {
  ...
  config.decorateComplexInterpolation = minapp.get(
    "decorateComplexInterpolation",
    true
  );
...
function getResolveRoots(doc: vscode.TextDocument) {
  let root = vscode.workspace.getWorkspaceFolder(
    doc.uri
  ) as vscode.WorkspaceFolder;
  return root
    ? config.resolveRoots.map((r) => path.resolve(root.uri.fsPath, r))
    : [];
}
