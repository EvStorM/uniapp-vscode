...
export default abstract class AutoCompletion {
  ...
  renderTag(tag: TagItem, sortText: string) {
    ...
    let attrs = allAttrs
      ...
      .map(
        (a, i) =>
          (isPug ? "" : " ") +
          `${a.name}=${attrQuote}${this.setDefault(
            i + 1,
            a.defaultValue
          )}${attrQuote}`
      );
    ...
    if (isPug) {
      ...
      if (this.config.selfCloseTags.includes(c.name)) {
        ...
        snippet = `${c.name}${attrs.join("")}${extraSpace}\${${len}}>\${${len +
          1}}</${c.name}>\${0}`;
      }
    }
  ...
  renderSnippet(
    ...
    item.documentation = new MarkdownString(
      snippet.markdown || snippet.description
    );
    item.sortText = sortText;
    return item;
  }
      ...
      ...Object.keys(allSnippets)
        ...
        .map((k) => {
          ...
          if (!snippet.description) {
            ...
            let found = res.natives.find(
              (it) => it.component.name === (ck || k)
            );
  ...
  async createComponentAttributeSnippetItems(
    ...
    if (tag.isOnAttrValue && tag.attrName) {
      ...
      if (tag.attrName === "class" || /^[\w\d-]+-class/.test(tag.attrName)) {
        ...
        return this.autoCompleteClassNames(
          doc,
          existsClass ? existsClass.trim().split(/\s+/) : []
        );
        ...
        if (
          ...
          let values = await autocompleteTagAttrValue(
            tag.name,
            tag.attrName,
            lc,
    ...
    if (!isEventPrefix && !lc.custom.hasOwnProperty(prefix)) {
      ...
      if (prefix.endsWith(".")) {
        if (lc.event.prefixes.some((p) => prefix.startsWith(p))) {
          ...
          let customPrefix = Object.keys(lc.custom).find((p) =>
            prefix.startsWith(p)
          );
      ...
      return modifiers.map(
        (m) => new CompletionItem(m, CompletionItemKind.Constant)
      );
    }

    let res = await autocompleteSpecialTagAttr(
      prefix,
      tag.name,
      tag.attrs,
      lc,
    ...
    stylefiles.forEach((stylefile, sfi) => {
      stylefile.styles.forEach((sty) => {
        if (!existsClassNames.includes(sty.name)) {
          ...
          i.detail = root
            ? path.relative(root, stylefile.file)
            : path.basename(stylefile.file);
   ...
   * @param pos
   */
  async createCloseTagCompletionItem(
    doc: TextDocument,
    pos: Position
  ...
  autoCompleteMethods(doc: TextDocument, prefix: string): CompletionItem[] {
    ...
    return methods.map((l) => {
      ...
      const filePath = root
        ? path.relative(root, l.loc.uri.fsPath)
        : path.basename(l.loc.uri.fsPath);
      // 低优先级排序滞后
