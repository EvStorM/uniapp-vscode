 ...
 * @LastEditors: E'vils
 * @LastEditTime: 2021-07-20 17:44:54
 * @Description:
 * @FilePath: /src/plugin/LinkProvider.ts
 */
/******************************************************************
...
export default class implements DocumentLinkProvider {
  ...
  private getLinks(doc: TextDocument) {
    ...
    let regexp = new RegExp(
      `\\b(${linkAttributeNames.join("|")})=['"]([^'"]+)['"]`,
      "g"
    );
    let remote = /^\w+:\/\//; // 是否是远程路径，如 "http://" ...
    doc
      .getText()
      .replace(regexp, (raw, tag: string, key: string, index: number) => {
        ...
        if (file) {
          ...
          links.push(
            new DocumentLink(
              ...
              isRemote ? Uri.parse(file) : Uri.file(file)
            )
          );
        }
        return raw;
